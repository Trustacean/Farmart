...
<body class="bg-gray-400">
  <div class="container mx-auto max-w-xl bg-background min-h-screen">
    <div class="p-4 flex justify-between items-center">
      <input type="text" placeholder="Search" class="w-3/4 p-2 rounded rounded-xl shadow drop-shadow-lg">
      <div class="flex space-x-4">
        <button >
          <svg class="h-8" viewBox="0 0 47 47" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path
              d="M20.375 32.8749C21.5256 32.8749 22.4584 31.9422 22.4584 30.7916C22.4584 29.641 21.5256 28.7083 20.375 28.7083C19.2244 28.7083 18.2917 29.641 18.2917 30.7916C18.2917 31.9422 19.2244 32.8749 20.375 32.8749Z"
              stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            <path
              d="M28.7083 32.8749C29.8589 32.8749 30.7917 31.9422 30.7917 30.7916C30.7917 29.641 29.8589 28.7083 28.7083 28.7083C27.5577 28.7083 26.625 29.641 26.625 30.7916C26.625 31.9422 27.5577 32.8749 28.7083 32.8749Z"
              stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            <path
              d="M14.125 14.125H16.2083L18.2917 26.625C18.5164 27.264 18.9426 27.8128 19.5063 28.1885C20.0699 28.5643 20.7404 28.7467 21.4167 28.7083H28.7083C29.3846 28.7467 30.0551 28.5643 30.6187 28.1885C31.1823 27.8128 31.6086 27.264 31.8333 26.625L32.875 19.3333H17.0417"
              stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </button>
        <div x-data="{ dropdownOpen: false }" @click.away="dropdownOpen = false" class="relative">
          ...
          <div x-show="dropdownOpen"
            ...
            <a href="{{ isset($user) ? './logout' : './login' }}"
              class="m-1 block px-4 py-2 text-sm capitalize {{ isset($user) ? 'hover:text-white hover:bg-red-600' : 'bg-white text-gray-700 hover:bg-primary hover:text-white' }} hover:text-white">
    ...
    <div class="text-center relative h-40">
      <img src="{{ asset('images/banner_image.png') }}" alt="Banner"
    ...
    <div class="flex flex-wrap justify-around p-4">
      <a href="{{ route('category', '1') }}" class="w-5/12 relative h-28 hover:scale-105 duration-500 rounded-lg m-2">
        <img src="{{ asset('images/nutri_tanaman.png') }}" alt="Nutri Tanaman"
          class="object-cover w-full h-full absolute inset-0 rounded-lg">
        <div class="flex justify-center items-center h-full w-full relative z-10 text-white font-bold">
          <p>Nutrisi Tanaman</p>
        </div>
      </a>
      <a href="{{ route('category', '2') }}" class="w-5/12 relative h-28 hover:scale-105 duration-500 rounded-lg m-2">
        <img src="{{ asset('images/bibit_beni.png') }}" alt="Bibit & Benih"
          class="object-cover w-full h-full absolute inset-0 rounded-lg">
        <div class="flex justify-center items-center h-full w-full relative z-10 text-white font-bold">
          <p>Bibit & Benih</p>
        </div>
      </a>
      <a href="{{ route('category', '3') }}" class="w-5/12 relative h-28 hover:scale-105 duration-500 rounded-lg m-2">
        <img src="{{ asset('images/proteksi_pertanian.png') }}" alt="Proteksi Tanaman"
          class="object-cover w-full h-full absolute inset-0 rounded-lg">
        <div class="flex justify-center items-center h-full w-full relative z-10 text-white font-bold">
          <p>Proteksi Tanaman</p>
        </div>
      </a>
      <a href="{{ route('category', '4') }}" class="w-5/12 relative h-28 hover:scale-105 duration-500 rounded-lg m-2">
        <img src="{{ asset('images/peralatan_pertanian.png') }}" alt="Peralatan Pertanian"
          class="object-cover w-full h-full absolute inset-0 rounded-lg">
        <div class="flex justify-center items-center h-full w-full relative z-10 text-white font-bold">
          <p>Peralatan Pertanian</p>
        </div>
      </a>
      <a href="{{ route('category', '5') }}" class="w-5/12 relative h-28 hover:scale-105 duration-500 rounded-lg m-2">
        <img src="{{ asset('images/teknologi_pertanian.png') }}" alt="Teknologi Pertanian"
          class="object-cover w-full h-full absolute inset-0 rounded-lg">
        <div class="flex justify-center items-center h-full w-full relative z-10 text-white font-bold">
          <p>Teknologi Pertanian</p>
        </div>
      </a>
      <a href="{{ route('category', '6') }}" class="w-5/12 relative h-28 hover:scale-105 duration-500 rounded-lg m-2">
        <img src="{{ asset('images/produk_peternakan.png') }}" alt="Produk Peternakan"
          class="object-cover w-full h-full absolute inset-0 rounded-lg">
        <div class="flex justify-center items-center h-full w-full relative z-10 text-white font-bold">
          <p>Produk Peternakan</p>
        </div>
      </a>
    </div>

    <div class="flex justify-center items-center bg-white p-1 h-full font-medium">
  <button class="flex-1 sm:flex-none w-full sm:w-auto bg-gray-300 hover:bg-primary hover:text-white hover:duration-300 p-2 m-1 rounded-lg h-full text-center">Rekomendasi</button>
  <button class="flex-1 sm:flex-none w-full sm:w-auto bg-gray-300 hover:bg-primary hover:text-white hover:duration-300 p-2 m-1 rounded-lg h-full text-center">Produk Baru</button>
  <button class="flex-1 sm:flex-none w-full sm:w-auto bg-gray-300 hover:bg-primary hover:text-white hover:duration-300 p-2 m-1 rounded-lg h-full text-center">Promo & Diskon</button>
</div>



    <div class="px-4">
      <h1 class="text-2xl font-bold p-2">Produk Terbaru</h1>
      @foreach($products as $product)
      <a class="flex bg-white shadow drop-shadow-lg rounded-lg p-4 mb-4"
